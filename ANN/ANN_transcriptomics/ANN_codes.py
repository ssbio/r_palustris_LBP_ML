...
df = pd.read_excel('transcriptomics_data.xlsx', sheet_name='Sheet1')
...
plt.title('Actual vs Predicted Growth Rate', fontsize=16, fontweight='bold')
plt.xlabel('Actual Growth Rate', fontsize=14, fontweight='bold')
plt.ylabel('Predicted Growth Rate', fontsize=14, fontweight='bold')
...
for i in range(len(residuals)):
    ...
    plt.title(f'Residuals - Sample {i+1}', fontweight='bold')
...
plt.ylabel('Gene Expression', fontsize=12, fontweight='bold')
...
cluster_map.ax_heatmap.set_yticklabels(df.iloc[:, 0], rotation=0, fontweight='bold')  # Assuming the actual conditions are in the first column
plt.title('Top 20 Most Important Genes', fontweight='bold')
plt.xlabel('Genes', fontweight='bold')
plt.ylabel('Conditions', fontweight='bold')  # Change y-axis label
plt.tight_layout()
plt.grid(False)
plt.show()
