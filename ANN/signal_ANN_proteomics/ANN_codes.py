...
model = MLPRegressor(hidden_layer_sizes=(200, 200, 200, 200, 200, 200, 200, 200),
                     max_iter=1000,
                     random_state=42)
...
for i in range(len(residuals)):
    ...
    plt.title(f'Residuals - Sample {i+1}')
...
top_proteins_perm = df.columns[1:9][top_indices_perm]
...
plt.title('Top 20 Most Important Proteins')
plt.xlabel('Proteins')
...
cluster_map.ax_heatmap.set_yticklabels(df.iloc[:, 0], rotation=0)  # Assuming the actual conditions are in the first column
plt.title('Clustermap for Top 20 Most Important Proteins')
plt.xlabel('Proteins')
plt.ylabel('Conditions')  # Change y-axis label
plt.tight_layout()
plt.grid(False)
plt.show()
